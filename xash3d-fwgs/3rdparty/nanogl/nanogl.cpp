...
#ifdef __webos__
// HP TouchPad: GLES 1.1 is libGLES_CM.so (Palm/Qualcomm naming, not Android's
// libGLESv1_CM.so). Only gl* symbols are pulled from it -- SDL owns the EGL
// context, and dlopening the lib does not touch the compositor.
...
{
#ifdef __webos__
	// HP TouchPad: GLES 1.1 lives in libGLES_CM.so (Palm/Qualcomm naming, not
	// the Android libGLESv1_CM.so). SDL owns the EGL context -- we only pull
	// gl* symbols here; dlopening the lib does NOT touch the compositor.
	const char *lib1 = "libGLES_CM.so";
	const char *lib2 = "libGLESv2.so";
	const char *lib3 = "libEGL.so";       // egl* fallback symbols only
#else
