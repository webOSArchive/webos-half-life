...
#ifdef __webos__
	// one-shot "no game data" notice (the app ships without Valve content;
	// New Game silently doing nothing reads as broken)
	CMenuYesNoMessageBox dataNotice { true };
	bool bDataChecked = false;
	// one-shot App Museum update notice; release builds draw no console
	// notify lines, so the engine's updater publishes through a cvar and
	// the menu is the user-facing surface
	CMenuYesNoMessageBox updateNotice { true };
	bool bUpdateShown = false;
#endif
};
...
{
#ifdef __webos__
	// gfx.wad ships with every Half-Life data set (demo and retail) but
	// never with the app itself, so its absence means "no game data yet"
	if( !bDataChecked )
	{
		bDataChecked = true;
		if( !EngFuncs::FileExists( "gfx.wad", false ))
		{
			dataNotice.SetMessage(
				"Half-Life game data not found.\n\n"
				"Connect the tablet to a computer, tap\n"
				"USB Drive, and copy your Half-Life\n"
				"'valve' folder into the 'xash' folder.\n\n"
	...
	if( !bUpdateShown )
	{
		const char *ver = EngFuncs::GetCvarString( "webos_update_version" );
		if( ver && ver[0] )
		{
			static char msg[128];
			snprintf( msg, sizeof( msg ),
				"Half-Life %s is available\nin the App Catalog.", ver );
			updateNotice.SetMessage( msg );
			updateNotice.Show();
			bUpdateShown = true;
		}
	}
