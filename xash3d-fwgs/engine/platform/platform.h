...
#ifdef __webos__
// direct-evdev gamepads + keyboards (webOS has no joydev and hides external
// keyboards from SDL); polled from the SDL1 backend's Platform_RunEvents
void Evdev_WebOS_Poll( void );
void Evdev_WebOS_Shutdown( void );
// App Museum II update check (background thread; publishes via cvar)
