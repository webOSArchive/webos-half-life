...
{
#ifdef __webos__
	// PDL must initialize BEFORE SDL_Init or GPU access via the 3-layer
	// compositor is not guaranteed. Order verified on hardware (sdlquake).
	if( PDL_Init( 0 ) != PDL_NOERROR )
		Sys_Warn( "PDL_Init failed (continuing)" );
	PDL_SetTouchAggression( PDL_AGGRESSION_MORETOUCHES );
	PDL_GesturesEnable( PDL_FALSE ); // system gesture layer eats screen-edge touches
#endif
