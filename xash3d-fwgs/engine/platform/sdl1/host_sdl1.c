...
#ifdef __webos__
		// The first-party HP Bluetooth keyboard comes through hidd -> Palm SDL
		// with private low keysyms for its special keys. Arrow order VERIFIED
		// on device 2026-08-15 by ordered-press capture: 18=Left 19=Up
		// 20=Right 21=Down. Each press still logs at dev level in case
		// another Palm keyboard disagrees.
		case 18: case 19: case 20: case 21:
		{
			static const int webos_arrows[] = { K_LEFTARROW, K_UPARROW, K_RIGHTARROW, K_DOWNARROW };
			keynum = webos_arrows[keynum - 18];
			if( down ) Con_Reportf( "webOS BT kbd sym %i -> %s\n",
			                        key.keysym.sym, Key_KeynumToString( keynum ));
			break;
		}
		case 24: return;   // PDLK_GESTURE_DISMISS_KEYBOARD
#endif
...
Palm's SDL 1.2 reports each finger as a separate SDL "mouse": the nonstandard
`which` field on mouse events is the contact id (0..4). Feed those into the
engine's touch subsystem -- IN_TouchEvent does UI_MouseMove BEFORE the click,
which is what makes a single tap activate a menu item (raw mouse events made
the first tap only hover), and in-game it drives the built-in touch overlay.
=============
*/
...
{
	float x, y;
} webos_finger[WEBOS_MAX_FINGERS];

static void SDLash_WebOSTouch( touchEventType type, int finger, int px, int py )
{
	float x, y, dx, dy;

	if( finger < 0 || finger >= WEBOS_MAX_FINGERS )
		return;
	if( !refState.width || !refState.height )
		return;

	x = px / (float)refState.width;
	y = py / (float)refState.height;

	if( type == event_motion )
	{
		dx = x - webos_finger[finger].x;
		dy = y - webos_finger[finger].y;
	}
	else
		dx = dy = 0.0f;

	webos_finger[finger].x = x;
	webos_finger[finger].y = y;

	IN_TouchEvent( type, finger, x, y, dx, dy );
}
#endif
...
static void SDLash_EventHandler( SDL_Event *event )
{
...
#ifdef __webos__
		SDLash_WebOSTouch( event_motion, event->motion.which, event->motion.x, event->motion.y );
#else
	...
	case SDL_MOUSEBUTTONDOWN:
		...
		if( event->button.button == SDL_BUTTON_LEFT )
			SDLash_WebOSTouch( event->type == SDL_MOUSEBUTTONDOWN ? event_down : event_up,
				event->button.which, event->button.x, event->button.y );
		else
			SDLash_MouseEvent( event->button ); // wheel etc. from a real BT mouse
#else
