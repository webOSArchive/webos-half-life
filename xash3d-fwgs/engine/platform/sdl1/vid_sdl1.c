...
static qboolean VID_CreateWindowWithSafeGL( const char *wndname, int xpos, int ypos, int w, int h, uint32_t flags )
{
	...
	{
#ifdef __webos__
		// TouchPad rule 2: ask for 0x0 "desktop mode" with plain SDL_OPENGL.
		// An explicit size (or SDL_FULLSCREEN/SDL_OPENGLES flags) yields a
		// 320x480 Palm-Pre-sized surface; 0x0 yields the native 1024x768 and
		// SDL sets the fullscreen/GLES flags itself.
		if( flags & SDL_OPENGL )
			host.hWnd = sw.surf = SDL_SetVideoMode( 0, 0, 0, SDL_OPENGL );
		else
...
#ifdef __webos__
	// The compositor decides the surface size (native 1024x768 in desktop
	// mode) -- record what we actually got, not what was asked for.
	if( sw.surf )
	{
		width = sw.surf->w;
		height = sw.surf->h;
		Con_Printf( "webOS: got %dx%d surface\n", width, height );
	}
#endif
...
{
	// A GL surface must be presented with SDL_GL_SwapBuffers; SDL_Flip is for
	// software surfaces. On Palm's SDL, Flip on a GL surface silently shows
	// nothing -- game runs, audio plays, screen stays black.
	if( glw_state.software )
		SDL_Flip( host.hWnd );
	else
		SDL_GL_SwapBuffers();
...
#ifdef __webos__
		// Palm's SDL 1.2 fork has these (stock 1.2 does not). Passing the
		// renderer's ES 1.1 request through is THE load-bearing line on the
		// TouchPad: without it Palm SDL requests an ES2 context and the
		// Adreno driver fails every config with EGL_BAD_ALLOC.
...
#ifndef __webos__
		// Palm's SDL 1.2 video driver has no dynamic-GL loader ("No dynamic
		// GL support in video driver") -- its GL is EGL-backed and always
		// present, and ref_gles1/NanoGL resolves gl* symbols itself from
		// libGLES_CM.so. Skip the load on webOS.
...
rserr_t R_ChangeDisplaySettings( int width, int height, window_mode_t window_mode )
{
	...
	Con_Reportf( "%s: Setting video mode to %dx%d %s\n", __func__, width, height, window_mode != WINDOW_MODE_WINDOWED ? "fullscreen" : "windowed" );
	...
	else if( window_mode != WINDOW_MODE_WINDOWED )
	{
...
/*
==================
webOS/SDL1 compat: symbols added upstream after the SDL1 backend bit-rotted.
The TouchPad is a fixed landscape 1024x768 panel with no gyro, so trivial
implementations are correct here, not placeholders.
==================
*/
void VID_Info_f( void )
{
	Con_Printf( "SDL1 backend: %dx%d\n", refState.width, refState.height );
}
