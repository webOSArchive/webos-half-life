...
static void Touch_InitConfig( void )
{
	...
	// a zero-length or truncated config (e.g. FAT leaves a 0-byte file after
	// power loss) execs to nothing, and Touch_WriteConfig never replaces a
	// config that produced no buttons — restore defaults instead of coming up
	// with an empty overlay (this also marks the config as changed so a clean
	// exit rewrites a valid file)
	if( !touch.list_user.first )
		Touch_LoadDefaults_f();
}
